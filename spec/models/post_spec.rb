...
RSpec.describe Post, type: :model do
  describe 'Validations Testing' do
    before(:all) do
      @user = User.new(name: 'Johnny Depp', posts_counter: 0)
      @user.save
    end

    subject do
      Post.new(title: 'Alice', text: 'in wonderland', comments_counter: 1, likes_counter: 0, author_id: @user.id)
    end

    before { subject.save }

    it 'should be valid if the title contains text' do
    ...
    it 'should not be valid if comments_counter is -7' do
    ...
    it 'should not be valid if likes_counter is string' do
      subject.likes_counter = 'bad'
      expect(subject).to_not be_valid
    end

    it 'should not be valid if likes_counter is -9' do
    ...
    it 'should return 5 comments when calling the most_recent_comments method' do
      (1..7).each do
        Comment.create(text: 'not this', author_id: @user.id, post_id: subject.id)
      end
